...
amount=0
asset="5500 ${policy_id}.${token_name}"
sc_asset="${amount} ${policy_id}.${token_name}"

min_value=$(${cli} transaction calculate-min-required-utxo \
    --babbage-era \
    --protocol-params-file tmp/protocol.json \
    --tx-out="${script_address} ${asset}" \
    --tx-out-inline-datum-file data/current_datum.json | tr -dc '0-9')
lock_value=$((${min_value} + 100000))

vestor_address_change="${vestor_address} + 5000000"
vestor_address_out="${vestor_address} + 5000000 + ${asset}"
...
FEE=$(${cli} transaction build \
    ...
    --spending-reference-tx-in-redeemer-file data/ban_redeemer.json \
